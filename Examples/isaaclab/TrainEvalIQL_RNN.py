"""Training and Evaluation script for IQL with Recurrent (GRU) models.

This script demonstrates how to use the IQLRecurrent algorithm with GRU-based
models and the SequentialRecurrentTrainer for offline reinforcement learning
...
def train_iql_rnn():
    ...
    # ============================================================
    image_mode = "None"  # Change this to switch modes
    ...
    # Create sequential datasets for GRU training
    dataset = HDF5RandomSequenceGRUDataset(
        file_path=data,
        sequence_length=sequence_length,
        min_idx=100,  # Skip first 100 episodes for training
        total_samples=50000,  # Virtual dataset size
    ...
    dataset_val = HDF5RandomSequenceGRUDataset(
        ...
        sequence_length=sequence_length,
        min_idx=1,
        max_idx=100,  # Use first 100 episodes for validation
        total_samples=5000,
        proprioceptive_keys=['angle_diff', 'distance', 'heading'],
        image_mode=image_mode,  # Pass image_mode to dataloader
    )
    ...
    print("Creating GRU models...")
    actor = GRUActorGaussian(**actor_config).to("cuda:0")
    value = GRUValue(**value_config).to("cuda:0")
    critic = GRUTwinQ(**critic_config).to("cuda:0")
    ...
    # Start training
    print("\n" + "="*60)
    print("Starting IQL-RNN Training")
    print("="*60)
    trainer.train()
...
def eval_rnn(trainer: Trainer):
    """Evaluate the trained recurrent policy."""
    env = load_isaaclab_env(task_name="AAURoverEnvRGBDRaw-v0")
    
    print("\n" + "="*60)
    print("Starting IQL-RNN Evaluation")
    print("="*60)
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("IQL with Recurrent (GRU) Models")
    print("="*60)
    
    trainer = train_iql_rnn()
    eval_rnn(trainer)
