...
class actor_gaussian_image(nn.Module):
    def __init__(self,
        ...
        self.encoder = ConvHeightmapEncoder(in_channels=heightmap_dim, encoder_features=image_encoder_features, encoder_activation=activation)
...
class conv_encoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Check if input is a sequence (batch_size, seq_len, channels, height, width)
...
class GRUActor(nn.Module):
    ...
    def forward(self, state, hidden=None):
        ...
        # Combine image and proprioceptive inputs-
        x = torch.cat([state["proprioceptive"][:, :, 2:4], x_img], dim=2)
        if hidden is None:
