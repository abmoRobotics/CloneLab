from typing import Dict

import torch
import torch.nn as nn
import torch.nn.functional as F
from skrl.models.torch.base import Model as BaseModel
from skrl.models.torch.gaussian import GaussianMixin


def get_activation(activation_name):
    activation_fns = {
        "leaky_relu": nn.LeakyReLU(),
        "relu": nn.ReLU(),
        "tanh": nn.Tanh(),
        "sigmoid": nn.Sigmoid(),
        "elu": nn.ELU(),
        "relu6": nn.ReLU6(),
        "selu": nn.SELU(),
    }
    if activation_name not in activation_fns:
        raise ValueError(
            f"Activation function {activation_name} not supported.")
    return activation_fns[activation_name]


class actor_gaussian_image(nn.Module):
    def __init__(self, 
                 device="cuda:0" if torch.cuda.is_available() else "cpu", 
                 proprioception_channels=3, 
                 encoder_channels=60, 
                 image_channels=1,
                 depth_channels=1,
                 action_dim=2,
                 mlp_features=[256, 160, 128],
                 image_input_dim=[224, 224],
                 image_encoder_features=[8, 16, 32, 64],
                 image_fc_features=[120, 60],
                 activation="leaky_relu",
                 dropout_rate=0,
                 use_batch_norm=False,
                 min_log_std=-20.0,
                 max_log_std=2.0,
                 state_independent_log_std=True):
        super(actor_gaussian_image, self).__init__()
        self.device = device
        self.proprioception_channels = proprioception_channels
        self.encoder_channels = encoder_channels
        self.action_dim = action_dim
        self.min_log_std = min_log_std
        self.max_log_std = max_log_std
        self.state_independent_log_std = state_independent_log_std

        self.image_encoder = conv_encoder(in_channels=image_channels, input_dim=image_input_dim, encoder_features=image_encoder_features, fc_features=image_fc_features, encoder_activation=activation)
        self.depth_encoder = conv_encoder(in_channels=depth_channels, input_dim=image_input_dim, encoder_features=image_encoder_features, fc_features=image_fc_features, encoder_activation=activation)

        self.mlp = nn.ModuleList()
        # Update encoder channels to be sum of both encoders
        self.encoder_channels = image_fc_features[-1] * 2
        in_channels = self.proprioception_channels + self.encoder_channels
        
        for feature in mlp_features:
            self.mlp.append(nn.Linear(in_channels, feature))
            if use_batch_norm:
                self.mlp.append(nn.BatchNorm1d(feature))
            self.mlp.append(get_activation(activation))
            if dropout_rate > 0:
                self.mlp.append(nn.Dropout(dropout_rate))
            in_channels = feature

        # Separate heads for mean and log_std
        self.mean_head = nn.Linear(in_channels, self.action_dim)
        if state_independent_log_std:
            self.log_std = nn.Parameter(torch.zeros(self.action_dim, dtype=torch.float32))
        else:
            self.log_std_head = nn.Linear(in_channels, self.action_dim)

        # Initialize weights properly
        self._init_weights()

    def _init_weights(self):
        """Initialize network weights using Xavier initialization"""
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)
                if m.bias is not None:
                    nn.init.zeros_(m.bias)
    
    def forward(self, state: Dict[str, torch.Tensor]):
        # Encode image
        x_img = self.image_encoder(state["image"])
        x_depth = self.depth_encoder(state["depth"])
        
        # Handle sequential vs batch data
        is_sequence = x_img.ndim == 3
        proprioceptive = state["proprioceptive"]
        
        if is_sequence:
            # For sequential data: (batch, seq_len, features)
            x = torch.cat([proprioceptive[..., :self.proprioception_channels], x_img, x_depth], dim=2)
        else:
            # For batch data: (batch, features)
            x = torch.cat([proprioceptive[..., :self.proprioception_channels], x_img, x_depth], dim=1)
        
        # Pass through MLP
        for layer in self.mlp:
            x = layer(x)
        
        # Get mean and log_std
        mu = torch.tanh(self.mean_head(x))  # Bound actions to [-1, 1]
        if self.state_independent_log_std:
            log_std = torch.clamp(self.log_std, min=self.min_log_std, max=self.max_log_std)
        else:
            log_std = self.log_std_head(x)
            log_std = torch.clamp(log_std, min=self.min_log_std, max=self.max_log_std)

        return torch.distributions.Normal(mu, log_std.exp())
    
    def get_action(self, state: Dict[str, torch.Tensor], deterministic=True):
        """Get action for deployment"""
        with torch.no_grad():
            dist = self.forward(state)
            if deterministic:
                return dist.mean
            else:
                return dist.sample()

class conv_encoder(nn.Module):
    def __init__(self, in_channels, input_dim=[224, 224], encoder_features=[8, 16, 32, 64], fc_features=[120, 60], encoder_activation="leaky_relu"):
        super().__init__()
        padding = 1
        stride = 1
        kernel_size = 3
        self.encoder_layers = nn.ModuleList()
        for feature in encoder_features:
            self.encoder_layers.append(
                nn.Conv2d(in_channels, feature, kernel_size=kernel_size, stride=stride, padding=padding, bias=False))
            self.encoder_layers.append(nn.BatchNorm2d(feature))
            self.encoder_layers.append(nn.LeakyReLU(inplace=True))
            self.encoder_layers.append(nn.Conv2d(feature, feature, kernel_size=kernel_size,
                                       stride=stride, padding=padding, bias=False))
            self.encoder_layers.append(nn.BatchNorm2d(feature))
            self.encoder_layers.append(nn.LeakyReLU(inplace=True))
            self.encoder_layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
            in_channels = feature

        self.fc_layers = nn.ModuleList()

        flattened_features = input_dim

        for _ in encoder_features:
            width = (flattened_features[0] - kernel_size + 2 * padding) // stride + 1
            height = (flattened_features[1] - kernel_size + 2 * padding) // stride + 1
            width = (width - 2) // 2 + 1
            height = (height - 2) // 2 + 1
            flattened_features = [width, height]

        in_channels = flattened_features[0] * flattened_features[1] * in_channels
        # Connect the convolutional layers to a fully connected layer
        for feature in fc_features:
            self.fc_layers.append(nn.Linear(in_channels, feature))
            self.fc_layers.append(nn.LeakyReLU())
            in_channels = feature

    def forward(self, x: torch.Tensor):

        is_sequence = x.ndim == 5 # Check if input is a sequence (batch_size, seq_len, channels, height, width)
        if is_sequence:
            # Input shape will be (B, T, C, H, W)
            batch_size, seq_len, channels, height, width = x.shape
            # Reshape to (B * T, C, H, W) for convolutional layers
            x = x.view(batch_size * seq_len, channels, height, width)

        # Apply the convolutional layers
        for layer in self.encoder_layers:
            x = layer(x)

        # Flatten the output for the fully connected layers
        x = x.reshape(x.size(0), -1)
        
        # Apply the fully connected layers
        for layer in self.fc_layers:
            x = layer(x)

        if is_sequence:
            # Reshape back to (B, T, features) if input was a sequence
            final_features = x.shape[1]
            x = x.view(batch_size, seq_len, final_features)
        return x

class v_image(nn.Module):
    def __init__(self, 
                 device="cuda:0" if torch.cuda.is_available() else "cpu", 
                 proprioception_channels=3, 
                 encoder_channels=60, 
                 image_channels=1,
                 depth_channels=1,
                 mlp_features=[256, 160, 128],
                 image_input_dim=[224, 224],
                 image_encoder_features=[8, 16, 32, 64],
                 image_fc_features=[120, 60],
                 activation="leaky_relu",
                 dropout_rate=0,
                 use_batch_norm=False,
                 **kwargs):
        super(v_image, self).__init__()
        self.device = device
        self.proprioception_channels = proprioception_channels
        self.encoder_channels = encoder_channels

        self.image_encoder = conv_encoder(in_channels=image_channels, input_dim=image_input_dim, encoder_features=image_encoder_features, fc_features=image_fc_features, encoder_activation=activation)
        self.depth_encoder = conv_encoder(in_channels=depth_channels, input_dim=image_input_dim, encoder_features=image_encoder_features, fc_features=image_fc_features, encoder_activation=activation)

        self.mlp = nn.ModuleList()
        # Update encoder channels
        self.encoder_channels = image_fc_features[-1] * 2
        in_channels = self.proprioception_channels + self.encoder_channels
        
        for feature in mlp_features:
            self.mlp.append(nn.Linear(in_channels, feature))
            if use_batch_norm:
                self.mlp.append(nn.BatchNorm1d(feature))
            self.mlp.append(get_activation(activation))
            if dropout_rate > 0:
                self.mlp.append(nn.Dropout(dropout_rate))
            in_channels = feature

        self.value_head = nn.Linear(in_channels, 1)
        
        # Initialize weights
        self._init_weights()
    
    def _init_weights(self):
        """Initialize network weights using Xavier initialization"""
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)
                if m.bias is not None:
                    nn.init.zeros_(m.bias)

    def forward(self, state: Dict[str, torch.Tensor]):
        # Encode image
        x_img = self.image_encoder(state["image"])
        x_depth = self.depth_encoder(state["depth"])
        
        # Handle sequential vs batch data
        is_sequence = x_img.ndim == 3
        proprioceptive = state["proprioceptive"]
        
        if is_sequence:
            # For sequential data: (batch, seq_len, features)
            x = torch.cat([proprioceptive[..., :self.proprioception_channels], x_img, x_depth], dim=2)
        else:
            # For batch data: (batch, features)
            x = torch.cat([proprioceptive[..., :self.proprioception_channels], x_img, x_depth], dim=1)
        
        # Pass through MLP
        for layer in self.mlp:
            x = layer(x)
        
        return self.value_head(x)

class q_image(nn.Module):
    def __init__(self, 
                 device="cuda:0" if torch.cuda.is_available() else "cpu", 
                 proprioception_channels=3, 
                 encoder_channels=60, 
                 image_channels=1,
                 depth_channels=1,
                 action_dim=2,
                 mlp_features=[256, 160, 128],
                 image_input_dim=[224, 224],
                 image_encoder_features=[8, 16, 32, 64],
                 image_fc_features=[120, 60],
                 activation="leaky_relu",
                 dropout_rate=0,
                 use_batch_norm=False):
        super(q_image, self).__init__()
        self.device = device
        self.proprioception_channels = proprioception_channels
        self.encoder_channels = encoder_channels
        self.action_dim = action_dim

        self.image_encoder = conv_encoder(in_channels=image_channels, input_dim=image_input_dim, encoder_features=image_encoder_features, fc_features=image_fc_features, encoder_activation=activation)
        self.depth_encoder = conv_encoder(in_channels=depth_channels, input_dim=image_input_dim, encoder_features=image_encoder_features, fc_features=image_fc_features, encoder_activation=activation)

        self.mlp = nn.ModuleList()
        # Update encoder channels
        self.encoder_channels = image_fc_features[-1] * 2
        # Include action dimensions in input
        in_channels = self.proprioception_channels + self.encoder_channels + self.action_dim
        
        for feature in mlp_features:
            self.mlp.append(nn.Linear(in_channels, feature))
            if use_batch_norm:
                self.mlp.append(nn.BatchNorm1d(feature))
            self.mlp.append(get_activation(activation))
            if dropout_rate > 0:
                self.mlp.append(nn.Dropout(dropout_rate))
            in_channels = feature

        self.q_head = nn.Linear(in_channels, 1)
        
        # Initialize weights
        self._init_weights()
    
    def _init_weights(self):
        """Initialize network weights using Xavier initialization"""
        pass
        # for m in self.modules():
        #     if isinstance(m, nn.Linear):
        #         nn.init.xavier_uniform_(m.weight)
        #         if m.bias is not None:
        #             nn.init.zeros_(m.bias)

    def forward(self, state: Dict[str, torch.Tensor], action: torch.Tensor):
        # Encode image
        x_img = self.image_encoder(state["image"])
        x_depth = self.depth_encoder(state["depth"])
        
        # Handle sequential vs batch data
        is_sequence = x_img.ndim == 3
        proprioceptive = state["proprioceptive"]

        if is_sequence:
            # For sequential data: (batch, seq_len, features)
            x = torch.cat([
                proprioceptive[..., :self.proprioception_channels], 
                x_img, 
                x_depth,
                action
            ], dim=2)
        else:
            # For batch data: (batch, features)
            x = torch.cat([
                proprioceptive[..., :self.proprioception_channels], 
                x_img, 
                x_depth,
                action
            ], dim=1)
        
        # Pass through MLP
        for layer in self.mlp:
            x = layer(x)
        
        return self.q_head(x)

class TwinQ_image(nn.Module):
    def __init__(self, 
                 device="cuda:0" if torch.cuda.is_available() else "cpu", 
                 proprioception_channels=3, 
                 encoder_channels=60, 
                 image_channels=1,
                 depth_channels=1,
                 action_dim=2,
                 **kwargs):
        super(TwinQ_image, self).__init__()
        self.device = device
        
        # Create two Q-networks with shared parameters for architecture
        q_kwargs = {
            'device': device,
            'proprioception_channels': proprioception_channels,
            'encoder_channels': encoder_channels,
            'image_channels': image_channels,
            'depth_channels': depth_channels,
            'action_dim': action_dim,
            **kwargs
        }
        
        self.q1 = q_image(**q_kwargs).to(device)
        self.q2 = q_image(**q_kwargs).to(device)

    def forward(self, state: Dict[str, torch.Tensor], action: torch.Tensor):
        q1 = self.q1(state, action)
        q2 = self.q2(state, action)
        return q1, q2
    
    def q1_forward(self, state: Dict[str, torch.Tensor], action: torch.Tensor):
        """Forward pass through Q1 only (useful for policy updates)"""
        return self.q1(state, action)

class GRUActor(nn.Module):
    def __init__(self, image_channels=4, image_size=[90, 160], proprioception_channels=3, encoder_channels=60, device=None):
        super(GRUActor, self).__init__()
        self.device = device or ("cuda:0" if torch.cuda.is_available() else "cpu")
        # Assuming conv_encoder is defined elsewhere
        self.encoder = conv_encoder(in_channels=image_channels, input_dim=image_size)
        self.hidden_size = 80
        self.num_layers = 2
        self.hidden_val = None
        self.gru = nn.GRU(input_size=proprioception_channels + encoder_channels,
                          hidden_size=self.hidden_size,
                          num_layers=self.num_layers,
                          batch_first=True)

        # Output layers
        mlp_features = [256, 160, 128]
        self.mlp = nn.ModuleList()
        in_features = self.hidden_size
        for out_features in mlp_features:
            self.mlp.append(nn.Linear(in_features, out_features))
            in_features = out_features

        self.mlp.append(nn.Linear(in_features, 2))
        self.mlp.append(nn.Tanh())

    def forward(self, state, hidden=None):
        # print(f'state shape: {state["image"].shape}')
        x_img = state["image"]
        batch, seq_len, channels, height, width = x_img.shape
        x_img = x_img.view(batch * seq_len, channels, height, width)
        x_img = self.encoder(x_img)
        x_img = x_img.view(batch, seq_len, -1)
        x = torch.cat([state["proprioceptive"][:, :, 2:4], x_img], dim=2)  # Combine image and proprioceptive inputs-
        if hidden is None:
            hidden = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(self.device)

        out, hidden = self.gru(x, hidden)  # OUT: (batch, seq, feature)
        # out = out[:, -1, :]  # Get the last output
        results = []
        for t in range(seq_len):
            step_out = out[:, t, :]
            for layer in self.mlp:
                step_out = layer(step_out)
            results.append(step_out)

        # for layer in self.mlp:
        #     out = layer(out)
        out = torch.stack(results, dim=1)
        return out

    def validate(self, state, done):
        with torch.no_grad():
            x_img = state["image"]
            if x_img.dim() == 4:  # Same check as in forward
                x_img = x_img.unsqueeze(1)
            batch, seq_len, channels, height, width = x_img.shape

            x_img = x_img.view(batch * seq_len, channels, height, width)
            x_img = self.encoder(x_img)
            x_img = x_img.view(batch, seq_len, -1)
            x_proprioceptive = state["proprioceptive"]
            if x_proprioceptive.dim() == 2:
                x_proprioceptive = x_proprioceptive.unsqueeze(1)
            x = torch.cat([x_proprioceptive[:, :, 2:4], x_img], dim=2)
            if self.hidden_val is None:
                self.hidden_val = torch.zeros(self.num_layers, batch, self.hidden_size).to(self.device)

            out, new_hidden = self.gru(x, self.hidden_val)
            not_done = ~done
            self.hidden_val = new_hidden * not_done.transpose(0, 1).unsqueeze(2).float()

            results = []
            for t in range(seq_len):
                step_out = out[:, t, :]
                for layer in self.mlp:
                    step_out = layer(step_out)
                results.append(step_out)

            out = torch.stack(results, dim=1)
            if out.dim() == 3:
                out = out.squeeze(1)
            return out


class GRUActorGaussian(nn.Module):
    """GRU-based actor with Gaussian output for stochastic policies.
    
    This model uses a GRU to process sequential observations and outputs
    a Gaussian distribution over actions for use in algorithms like SAC or PPO.
    
    Args:
        image_channels: Number of input image channels.
        depth_channels: Number of depth image channels.
        image_size: Size of input images [H, W].
        proprioception_channels: Number of proprioceptive input features.
        action_dim: Dimension of the action space.
        hidden_size: Size of GRU hidden state.
        num_layers: Number of GRU layers.
        image_encoder_features: List of features for conv encoder layers.
        image_fc_features: List of features for encoder FC layers.
        mlp_features: List of features for output MLP layers.
        min_log_std: Minimum log standard deviation.
        max_log_std: Maximum log standard deviation.
        state_independent_log_std: Whether to use state-independent log std.
        device: Device to use for computation.
    """
    
    def __init__(
        self,
        image_channels: int = 3,
        depth_channels: int = 1,
        image_size: list = [224, 224],
        proprioception_channels: int = 3,
        action_dim: int = 2,
        hidden_size: int = 128,
        num_layers: int = 2,
        image_encoder_features: list = [8, 16, 32, 64],
        image_fc_features: list = [120, 60],
        mlp_features: list = [256, 160, 128],
        min_log_std: float = -20.0,
        max_log_std: float = 2.0,
        state_independent_log_std: bool = True,
        device: str = None,
    ):
        super(GRUActorGaussian, self).__init__()
        self.device = device or ("cuda:0" if torch.cuda.is_available() else "cpu")
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.hidden_val = None
        self.action_dim = action_dim
        self.min_log_std = min_log_std
        self.max_log_std = max_log_std
        self.state_independent_log_std = state_independent_log_std
        self.proprioception_channels = proprioception_channels
        self.image_channels = image_channels
        self.depth_channels = depth_channels

        # Image encoders (optional based on channels)
        encoder_output_size = 0
        if image_channels > 0:
            self.image_encoder = conv_encoder(
                in_channels=image_channels,
                input_dim=image_size,
                encoder_features=image_encoder_features,
                fc_features=image_fc_features
            )
            encoder_output_size += image_fc_features[-1]
        else:
            self.image_encoder = None
            
        if depth_channels > 0:
            self.depth_encoder = conv_encoder(
                in_channels=depth_channels,
                input_dim=image_size,
                encoder_features=image_encoder_features,
                fc_features=image_fc_features
            )
            encoder_output_size += image_fc_features[-1]
        else:
            self.depth_encoder = None

        # GRU input size: proprioception + image features + depth features
        gru_input_size = proprioception_channels + encoder_output_size

        self.gru = nn.GRU(
            input_size=gru_input_size,
            hidden_size=hidden_size,
            num_layers=num_layers,
            batch_first=True
        )

        # Output MLP
        self.mlp = nn.ModuleList()
        in_features = hidden_size
        for out_features in mlp_features:
            self.mlp.append(nn.Linear(in_features, out_features))
            self.mlp.append(nn.LeakyReLU())
            in_features = out_features

        # Output heads
        self.mean_head = nn.Linear(in_features, action_dim)
        if state_independent_log_std:
            self.log_std = nn.Parameter(torch.zeros(action_dim, dtype=torch.float32))
        else:
            self.log_std_head = nn.Linear(in_features, action_dim)

        self._init_weights()

    def _init_weights(self):
        """Initialize network weights using Xavier initialization"""
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)
                if m.bias is not None:
                    nn.init.zeros_(m.bias)

    def forward(self, state: Dict[str, torch.Tensor], hidden: torch.Tensor = None):
        """Forward pass for training with sequences.
        
        Args:
            state: Dictionary containing 'image', 'depth', and 'proprioceptive' tensors.
                   Images should have shape (batch, seq_len, channels, height, width).
                   'image' and 'depth' are optional if respective channels are 0.
            hidden: Optional hidden state for GRU.
            
        Returns:
            dist: Normal distribution over actions.
            hidden: Updated hidden state.
        """
        proprioceptive = state["proprioceptive"]
        
        # Handle both batched sequences and single timesteps
        if proprioceptive.dim() == 2:
            proprioceptive = proprioceptive.unsqueeze(1)
        
        batch = proprioceptive.shape[0]
        seq_len = proprioceptive.shape[1]
        
        # Build feature list starting with proprioception
        features = [proprioceptive[..., :self.proprioception_channels]]
        
        # Encode images if image encoder exists
        if self.image_encoder is not None and "image" in state:
            x_img = state["image"]
            if x_img.dim() == 4:
                x_img = x_img.unsqueeze(1)
            _, _, channels, height, width = x_img.shape
            x_img = x_img.view(batch * seq_len, channels, height, width)
            x_img = self.image_encoder(x_img)
            x_img = x_img.view(batch, seq_len, -1)
            features.append(x_img)
        
        # Encode depth if depth encoder exists
        if self.depth_encoder is not None and "depth" in state:
            x_depth = state["depth"]
            if x_depth.dim() == 4:
                x_depth = x_depth.unsqueeze(1)
            _, _, channels, height, width = x_depth.shape
            x_depth = x_depth.view(batch * seq_len, channels, height, width)
            x_depth = self.depth_encoder(x_depth)
            x_depth = x_depth.view(batch, seq_len, -1)
            features.append(x_depth)

        # Concatenate features
        x = torch.cat(features, dim=2)

        # Initialize hidden state if not provided
        if hidden is None:
            hidden = torch.zeros(self.num_layers, batch, self.hidden_size).to(self.device)

        # GRU forward
        out, hidden = self.gru(x, hidden)

        # Process each timestep through MLP
        results = []
        for t in range(seq_len):
            step_out = out[:, t, :]
            for layer in self.mlp:
                step_out = layer(step_out)
            results.append(step_out)

        out = torch.stack(results, dim=1)

        # Get mean and log_std
        mu = torch.tanh(self.mean_head(out))
        if self.state_independent_log_std:
            log_std = torch.clamp(self.log_std, min=self.min_log_std, max=self.max_log_std)
            log_std = log_std.expand_as(mu)
        else:
            log_std = self.log_std_head(out)
            log_std = torch.clamp(log_std, min=self.min_log_std, max=self.max_log_std)

        return torch.distributions.Normal(mu, log_std.exp()), hidden

    def get_action(self, state: Dict[str, torch.Tensor], deterministic: bool = True):
        """Get action for deployment (handles hidden state internally)."""
        with torch.no_grad():
            dist, self.hidden_val = self.forward(state, self.hidden_val)
            if deterministic:
                return dist.mean.squeeze(1) if dist.mean.dim() == 3 else dist.mean
            else:
                action = dist.sample()
                return action.squeeze(1) if action.dim() == 3 else action

    def reset_hidden(self, batch_size: int = 1):
        """Reset hidden state for new episodes."""
        self.hidden_val = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(self.device)

    def validate(self, state: Dict[str, torch.Tensor], done: torch.Tensor):
        """Validation forward pass with hidden state management based on done flags."""
        with torch.no_grad():
            x_img = state["image"]
            x_depth = state["depth"]
            proprioceptive = state["proprioceptive"]

            if x_img.dim() == 4:
                x_img = x_img.unsqueeze(1)
                x_depth = x_depth.unsqueeze(1)
                proprioceptive = proprioceptive.unsqueeze(1)

            batch, seq_len, channels, height, width = x_img.shape

            # Encode images
            x_img = x_img.view(batch * seq_len, channels, height, width)
            x_img = self.image_encoder(x_img)
            x_img = x_img.view(batch, seq_len, -1)

            x_depth = x_depth.view(batch * seq_len, -1, height, width)
            x_depth = self.depth_encoder(x_depth)
            x_depth = x_depth.view(batch, seq_len, -1)

            x = torch.cat([
                proprioceptive[..., :self.proprioception_channels],
                x_img,
                x_depth
            ], dim=2)

            if self.hidden_val is None:
                self.hidden_val = torch.zeros(self.num_layers, batch, self.hidden_size).to(self.device)

            out, new_hidden = self.gru(x, self.hidden_val)

            # Reset hidden state where done is True
            # Handle different done tensor shapes: (batch,), (batch, 1), etc.
            if done is not None:
                # Ensure done is a 1D boolean tensor of shape (batch,)
                done_flat = done.view(-1) if done.dim() > 1 else done
                not_done = ~done_flat.bool()
                # Expand to match hidden state shape: (num_layers, batch, hidden_size)
                not_done_expanded = not_done.unsqueeze(0).unsqueeze(2).expand_as(new_hidden)
                self.hidden_val = new_hidden * not_done_expanded.float()
            else:
                self.hidden_val = new_hidden

            # Process through MLP
            results = []
            for t in range(seq_len):
                step_out = out[:, t, :]
                for layer in self.mlp:
                    step_out = layer(step_out)
                results.append(step_out)

            out = torch.stack(results, dim=1)
            mu = torch.tanh(self.mean_head(out))

            if out.dim() == 3 and out.shape[1] == 1:
                mu = mu.squeeze(1)

            return mu


class GRUValue(nn.Module):
    """GRU-based value function (V) for sequential observations.
    
    Args:
        image_channels: Number of input image channels.
        depth_channels: Number of depth image channels.
        image_size: Size of input images [H, W].
        proprioception_channels: Number of proprioceptive input features.
        hidden_size: Size of GRU hidden state.
        num_layers: Number of GRU layers.
        image_encoder_features: List of features for conv encoder layers.
        image_fc_features: List of features for encoder FC layers.
        mlp_features: List of features for output MLP layers.
        device: Device to use for computation.
    """
    
    def __init__(
        self,
        image_channels: int = 3,
        depth_channels: int = 1,
        image_size: list = [224, 224],
        proprioception_channels: int = 3,
        hidden_size: int = 128,
        num_layers: int = 2,
        image_encoder_features: list = [8, 16, 32, 64],
        image_fc_features: list = [120, 60],
        mlp_features: list = [256, 160, 128],
        device: str = None,
    ):
        super(GRUValue, self).__init__()
        self.device = device or ("cuda:0" if torch.cuda.is_available() else "cpu")
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.hidden_val = None
        self.proprioception_channels = proprioception_channels
        self.image_channels = image_channels
        self.depth_channels = depth_channels

        # Image encoders (optional based on channels)
        encoder_output_size = 0
        if image_channels > 0:
            self.image_encoder = conv_encoder(
                in_channels=image_channels,
                input_dim=image_size,
                encoder_features=image_encoder_features,
                fc_features=image_fc_features
            )
            encoder_output_size += image_fc_features[-1]
        else:
            self.image_encoder = None
            
        if depth_channels > 0:
            self.depth_encoder = conv_encoder(
                in_channels=depth_channels,
                input_dim=image_size,
                encoder_features=image_encoder_features,
                fc_features=image_fc_features
            )
            encoder_output_size += image_fc_features[-1]
        else:
            self.depth_encoder = None

        # GRU
        gru_input_size = proprioception_channels + encoder_output_size

        self.gru = nn.GRU(
            input_size=gru_input_size,
            hidden_size=hidden_size,
            num_layers=num_layers,
            batch_first=True
        )

        # Output MLP
        self.mlp = nn.ModuleList()
        in_features = hidden_size
        for out_features in mlp_features:
            self.mlp.append(nn.Linear(in_features, out_features))
            self.mlp.append(nn.LeakyReLU())
            in_features = out_features

        self.value_head = nn.Linear(in_features, 1)
        self._init_weights()

    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)
                if m.bias is not None:
                    nn.init.zeros_(m.bias)

    def forward(self, state: Dict[str, torch.Tensor], hidden: torch.Tensor = None):
        """Forward pass for value estimation.
        
        Args:
            state: Dictionary containing 'image', 'depth', and 'proprioceptive' tensors.
                   'image' and 'depth' are optional if respective channels are 0.
            hidden: Optional hidden state for GRU.
            
        Returns:
            value: Value estimates with shape (batch, seq_len, 1).
            hidden: Updated hidden state.
        """
        proprioceptive = state["proprioceptive"]
        
        # Handle both batched sequences and single timesteps
        if proprioceptive.dim() == 2:
            proprioceptive = proprioceptive.unsqueeze(1)
        
        batch = proprioceptive.shape[0]
        seq_len = proprioceptive.shape[1]
        
        # Build feature list starting with proprioception
        features = [proprioceptive[..., :self.proprioception_channels]]
        
        # Encode images if image encoder exists
        if self.image_encoder is not None and "image" in state:
            x_img = state["image"]
            if x_img.dim() == 4:
                x_img = x_img.unsqueeze(1)
            _, _, channels, height, width = x_img.shape
            x_img = x_img.view(batch * seq_len, channels, height, width)
            x_img = self.image_encoder(x_img)
            x_img = x_img.view(batch, seq_len, -1)
            features.append(x_img)
        
        # Encode depth if depth encoder exists
        if self.depth_encoder is not None and "depth" in state:
            x_depth = state["depth"]
            if x_depth.dim() == 4:
                x_depth = x_depth.unsqueeze(1)
            _, _, channels, height, width = x_depth.shape
            x_depth = x_depth.view(batch * seq_len, channels, height, width)
            x_depth = self.depth_encoder(x_depth)
            x_depth = x_depth.view(batch, seq_len, -1)
            features.append(x_depth)

        x = torch.cat(features, dim=2)

        if hidden is None:
            hidden = torch.zeros(self.num_layers, batch, self.hidden_size).to(self.device)

        out, hidden = self.gru(x, hidden)

        # Process each timestep
        results = []
        for t in range(seq_len):
            step_out = out[:, t, :]
            for layer in self.mlp:
                step_out = layer(step_out)
            results.append(step_out)

        out = torch.stack(results, dim=1)
        value = self.value_head(out)

        return value, hidden

    def reset_hidden(self, batch_size: int = 1):
        self.hidden_val = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(self.device)


class GRUQNetwork(nn.Module):
    """GRU-based Q-function for sequential observations.
    
    Args:
        image_channels: Number of input image channels.
        depth_channels: Number of depth image channels.
        image_size: Size of input images [H, W].
        proprioception_channels: Number of proprioceptive input features.
        action_dim: Dimension of the action space.
        hidden_size: Size of GRU hidden state.
        num_layers: Number of GRU layers.
        image_encoder_features: List of features for conv encoder layers.
        image_fc_features: List of features for encoder FC layers.
        mlp_features: List of features for output MLP layers.
        device: Device to use for computation.
    """
    
    def __init__(
        self,
        image_channels: int = 3,
        depth_channels: int = 1,
        image_size: list = [224, 224],
        proprioception_channels: int = 3,
        action_dim: int = 2,
        hidden_size: int = 128,
        num_layers: int = 2,
        image_encoder_features: list = [8, 16, 32, 64],
        image_fc_features: list = [120, 60],
        mlp_features: list = [256, 160, 128],
        device: str = None,
    ):
        super(GRUQNetwork, self).__init__()
        self.device = device or ("cuda:0" if torch.cuda.is_available() else "cpu")
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.hidden_val = None
        self.proprioception_channels = proprioception_channels
        self.action_dim = action_dim
        self.image_channels = image_channels
        self.depth_channels = depth_channels

        # Image encoders (optional based on channels)
        encoder_output_size = 0
        if image_channels > 0:
            self.image_encoder = conv_encoder(
                in_channels=image_channels,
                input_dim=image_size,
                encoder_features=image_encoder_features,
                fc_features=image_fc_features
            )
            encoder_output_size += image_fc_features[-1]
        else:
            self.image_encoder = None
            
        if depth_channels > 0:
            self.depth_encoder = conv_encoder(
                in_channels=depth_channels,
                input_dim=image_size,
                encoder_features=image_encoder_features,
                fc_features=image_fc_features
            )
            encoder_output_size += image_fc_features[-1]
        else:
            self.depth_encoder = None

        # GRU - includes action in input
        gru_input_size = proprioception_channels + encoder_output_size + action_dim

        self.gru = nn.GRU(
            input_size=gru_input_size,
            hidden_size=hidden_size,
            num_layers=num_layers,
            batch_first=True
        )

        # Output MLP
        self.mlp = nn.ModuleList()
        in_features = hidden_size
        for out_features in mlp_features:
            self.mlp.append(nn.Linear(in_features, out_features))
            self.mlp.append(nn.LeakyReLU())
            in_features = out_features

        self.q_head = nn.Linear(in_features, 1)
        self._init_weights()

    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)
                if m.bias is not None:
                    nn.init.zeros_(m.bias)

    def forward(self, state: Dict[str, torch.Tensor], action: torch.Tensor, hidden: torch.Tensor = None):
        """Forward pass for Q-value estimation.
        
        Args:
            state: Dictionary containing 'image', 'depth', and 'proprioceptive' tensors.
                   'image' and 'depth' are optional if respective channels are 0.
            action: Action tensor with shape (batch, seq_len, action_dim) or (batch, action_dim).
            hidden: Optional hidden state for GRU.
            
        Returns:
            q_value: Q-value estimates with shape (batch, seq_len, 1).
            hidden: Updated hidden state.
        """
        proprioceptive = state["proprioceptive"]
        
        # Handle both batched sequences and single timesteps
        if proprioceptive.dim() == 2:
            proprioceptive = proprioceptive.unsqueeze(1)
            action = action.unsqueeze(1)
        
        batch = proprioceptive.shape[0]
        seq_len = proprioceptive.shape[1]
        
        # Build feature list starting with proprioception
        features = [proprioceptive[..., :self.proprioception_channels]]
        
        # Encode images if image encoder exists
        if self.image_encoder is not None and "image" in state:
            x_img = state["image"]
            if x_img.dim() == 4:
                x_img = x_img.unsqueeze(1)
            _, _, channels, height, width = x_img.shape
            x_img = x_img.view(batch * seq_len, channels, height, width)
            x_img = self.image_encoder(x_img)
            x_img = x_img.view(batch, seq_len, -1)
            features.append(x_img)
        
        # Encode depth if depth encoder exists
        if self.depth_encoder is not None and "depth" in state:
            x_depth = state["depth"]
            if x_depth.dim() == 4:
                x_depth = x_depth.unsqueeze(1)
            _, _, channels, height, width = x_depth.shape
            x_depth = x_depth.view(batch * seq_len, channels, height, width)
            x_depth = self.depth_encoder(x_depth)
            x_depth = x_depth.view(batch, seq_len, -1)
            features.append(x_depth)
        
        # Add action
        features.append(action)

        x = torch.cat(features, dim=2)

        if hidden is None:
            hidden = torch.zeros(self.num_layers, batch, self.hidden_size).to(self.device)

        out, hidden = self.gru(x, hidden)

        # Process each timestep
        results = []
        for t in range(seq_len):
            step_out = out[:, t, :]
            for layer in self.mlp:
                step_out = layer(step_out)
            results.append(step_out)

        out = torch.stack(results, dim=1)
        q_value = self.q_head(out)

        return q_value, hidden

    def reset_hidden(self, batch_size: int = 1):
        self.hidden_val = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(self.device)


class GRUTwinQ(nn.Module):
    """Twin GRU Q-networks for algorithms like SAC and IQL.
    
    Args:
        **kwargs: Arguments passed to GRUQNetwork.
    """
    
    def __init__(self, **kwargs):
        super(GRUTwinQ, self).__init__()
        self.device = kwargs.get('device', "cuda:0" if torch.cuda.is_available() else "cpu")
        
        self.q1 = GRUQNetwork(**kwargs)
        self.q2 = GRUQNetwork(**kwargs)

    def forward(self, state: Dict[str, torch.Tensor], action: torch.Tensor, 
                hidden1: torch.Tensor = None, hidden2: torch.Tensor = None):
        """Forward pass through both Q-networks.
        
        Returns:
            q1, q2: Q-value estimates from both networks.
            hidden1, hidden2: Updated hidden states.
        """
        q1, hidden1 = self.q1(state, action, hidden1)
        q2, hidden2 = self.q2(state, action, hidden2)
        return q1, q2, hidden1, hidden2

    def q1_forward(self, state: Dict[str, torch.Tensor], action: torch.Tensor, hidden: torch.Tensor = None):
        """Forward pass through Q1 only."""
        return self.q1(state, action, hidden)

    def reset_hidden(self, batch_size: int = 1):
        self.q1.reset_hidden(batch_size)
        self.q2.reset_hidden(batch_size)


class GRUActorCritic(nn.Module):
    """Combined GRU Actor-Critic with shared encoder for efficiency.
    
    This model shares the image encoders between actor and critic to reduce
    computational cost while maintaining separate GRU and MLP heads.
    
    Args:
        image_channels: Number of input image channels.
        depth_channels: Number of depth image channels.
        image_size: Size of input images [H, W].
        proprioception_channels: Number of proprioceptive input features.
        action_dim: Dimension of the action space.
        hidden_size: Size of GRU hidden state.
        num_layers: Number of GRU layers.
        image_encoder_features: List of features for conv encoder layers.
        image_fc_features: List of features for encoder FC layers.
        mlp_features: List of features for output MLP layers.
        min_log_std: Minimum log standard deviation.
        max_log_std: Maximum log standard deviation.
        device: Device to use for computation.
    """
    
    def __init__(
        self,
        image_channels: int = 3,
        depth_channels: int = 1,
        image_size: list = [224, 224],
        proprioception_channels: int = 3,
        action_dim: int = 2,
        hidden_size: int = 128,
        num_layers: int = 2,
        image_encoder_features: list = [8, 16, 32, 64],
        image_fc_features: list = [120, 60],
        mlp_features: list = [256, 160, 128],
        min_log_std: float = -20.0,
        max_log_std: float = 2.0,
        device: str = None,
    ):
        super(GRUActorCritic, self).__init__()
        self.device = device or ("cuda:0" if torch.cuda.is_available() else "cpu")
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.action_dim = action_dim
        self.min_log_std = min_log_std
        self.max_log_std = max_log_std
        self.proprioception_channels = proprioception_channels
        self.image_channels = image_channels
        self.depth_channels = depth_channels

        # Hidden states
        self.actor_hidden = None
        self.critic_hidden = None

        # Shared image encoders (optional based on channels)
        encoder_output_size = 0
        if image_channels > 0:
            self.image_encoder = conv_encoder(
                in_channels=image_channels,
                input_dim=image_size,
                encoder_features=image_encoder_features,
                fc_features=image_fc_features
            )
            encoder_output_size += image_fc_features[-1]
        else:
            self.image_encoder = None
            
        if depth_channels > 0:
            self.depth_encoder = conv_encoder(
                in_channels=depth_channels,
                input_dim=image_size,
                encoder_features=image_encoder_features,
                fc_features=image_fc_features
            )
            encoder_output_size += image_fc_features[-1]
        else:
            self.depth_encoder = None

        # GRU input size
        gru_input_size = proprioception_channels + encoder_output_size

        # Separate GRUs for actor and critic
        self.actor_gru = nn.GRU(
            input_size=gru_input_size,
            hidden_size=hidden_size,
            num_layers=num_layers,
            batch_first=True
        )
        self.critic_gru = nn.GRU(
            input_size=gru_input_size,
            hidden_size=hidden_size,
            num_layers=num_layers,
            batch_first=True
        )

        # Actor MLP
        self.actor_mlp = nn.ModuleList()
        in_features = hidden_size
        for out_features in mlp_features:
            self.actor_mlp.append(nn.Linear(in_features, out_features))
            self.actor_mlp.append(nn.LeakyReLU())
            in_features = out_features

        self.mean_head = nn.Linear(in_features, action_dim)
        self.log_std = nn.Parameter(torch.zeros(action_dim, dtype=torch.float32))

        # Critic MLP
        self.critic_mlp = nn.ModuleList()
        in_features = hidden_size
        for out_features in mlp_features:
            self.critic_mlp.append(nn.Linear(in_features, out_features))
            self.critic_mlp.append(nn.LeakyReLU())
            in_features = out_features

        self.value_head = nn.Linear(in_features, 1)

        self._init_weights()

    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)
                if m.bias is not None:
                    nn.init.zeros_(m.bias)

    def _encode_observations(self, state: Dict[str, torch.Tensor]):
        """Encode observations using shared encoders."""
        proprioceptive = state["proprioceptive"]

        # Handle both batched sequences and single timesteps
        if proprioceptive.dim() == 2:
            proprioceptive = proprioceptive.unsqueeze(1)
        
        batch = proprioceptive.shape[0]
        seq_len = proprioceptive.shape[1]
        
        # Build feature list starting with proprioception
        features = [proprioceptive[..., :self.proprioception_channels]]
        
        # Encode images if image encoder exists
        if self.image_encoder is not None and "image" in state:
            x_img = state["image"]
            if x_img.dim() == 4:
                x_img = x_img.unsqueeze(1)
            _, _, channels, height, width = x_img.shape
            x_img = x_img.view(batch * seq_len, channels, height, width)
            x_img = self.image_encoder(x_img)
            x_img = x_img.view(batch, seq_len, -1)
            features.append(x_img)
        
        # Encode depth if depth encoder exists
        if self.depth_encoder is not None and "depth" in state:
            x_depth = state["depth"]
            if x_depth.dim() == 4:
                x_depth = x_depth.unsqueeze(1)
            _, _, channels, height, width = x_depth.shape
            x_depth = x_depth.view(batch * seq_len, channels, height, width)
            x_depth = self.depth_encoder(x_depth)
            x_depth = x_depth.view(batch, seq_len, -1)
            features.append(x_depth)

        x = torch.cat(features, dim=2)

        return x, batch, seq_len

    def forward_actor(self, state: Dict[str, torch.Tensor], hidden: torch.Tensor = None):
        """Forward pass for actor."""
        x, batch, seq_len = self._encode_observations(state)

        if hidden is None:
            hidden = torch.zeros(self.num_layers, batch, self.hidden_size).to(self.device)

        out, hidden = self.actor_gru(x, hidden)

        results = []
        for t in range(seq_len):
            step_out = out[:, t, :]
            for layer in self.actor_mlp:
                step_out = layer(step_out)
            results.append(step_out)

        out = torch.stack(results, dim=1)

        mu = torch.tanh(self.mean_head(out))
        log_std = torch.clamp(self.log_std, min=self.min_log_std, max=self.max_log_std)
        log_std = log_std.expand_as(mu)

        return torch.distributions.Normal(mu, log_std.exp()), hidden

    def forward_critic(self, state: Dict[str, torch.Tensor], hidden: torch.Tensor = None):
        """Forward pass for critic."""
        x, batch, seq_len = self._encode_observations(state)

        if hidden is None:
            hidden = torch.zeros(self.num_layers, batch, self.hidden_size).to(self.device)

        out, hidden = self.critic_gru(x, hidden)

        results = []
        for t in range(seq_len):
            step_out = out[:, t, :]
            for layer in self.critic_mlp:
                step_out = layer(step_out)
            results.append(step_out)

        out = torch.stack(results, dim=1)
        value = self.value_head(out)

        return value, hidden

    def forward(self, state: Dict[str, torch.Tensor], actor_hidden: torch.Tensor = None, critic_hidden: torch.Tensor = None):
        """Forward pass for both actor and critic."""
        dist, actor_hidden = self.forward_actor(state, actor_hidden)
        value, critic_hidden = self.forward_critic(state, critic_hidden)
        return dist, value, actor_hidden, critic_hidden

    def get_action(self, state: Dict[str, torch.Tensor], deterministic: bool = True):
        """Get action for deployment."""
        with torch.no_grad():
            dist, self.actor_hidden = self.forward_actor(state, self.actor_hidden)
            if deterministic:
                action = dist.mean
            else:
                action = dist.sample()
            return action.squeeze(1) if action.dim() == 3 and action.shape[1] == 1 else action

    def reset_hidden(self, batch_size: int = 1):
        """Reset hidden states for new episodes."""
        self.actor_hidden = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(self.device)
        self.critic_hidden = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(self.device)