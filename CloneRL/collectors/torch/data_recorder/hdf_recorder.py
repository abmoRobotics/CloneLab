...
class HDF5DataRecorder(DataRecorderBase):
    ...
    def _create_new_file(self) -> None:
        ...
        self.file_name = f"{self.base_filename}_{self.current_file_index}.hdf5"
        self.current_file_index += 1

        with h5py.File(self.file_name, "w") as file:
            file.create_dataset("observations", (self.max_rows,
                                self.num_observations), dtype=self.env.observation_space.dtype)
            file.create_dataset(
                "actions", (self.max_rows, self.num_actions), dtype=self.env.action_space.dtype)
            file.create_dataset(
                "rewards", (self.max_rows, 1), dtype=np.float32)
    ...
    def write_to_disk(self, rover_id: int) -> None:
        data_chunk = {key: np.array(
            value) for key, value in self.data_buffers[rover_id].items()}
