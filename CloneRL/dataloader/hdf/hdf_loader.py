...
class HDF5DictDatasetRandom(Dataset):
    ...
    def __init__(self, file_path: str, min_idx=0, max_idx=None, total_samples=120000, 
        ...
        with h5py.File(self.file_path, 'r') as file:
            ...
            for demo_key in demo_keys:
                ...
                # Start from min_history_timestep to ensure we have enough frames for stacking
                # Exclude the last timestep to ensure we have valid next_obs
                for timestep in range(self.min_history_timestep, num_samples - 1):
    ...
    def _extract_obs_stacked(self, obs_group, timestep):
        """Extract observations with strided frame stacking for temporal context.
        
        Returns stacked frames at indices [t - (n-1)*stride, t - (n-2)*stride, ..., t]
        For num_stacked_frames=3 and stride=3: [t-6, t-3, t]
        """
        # Calculate the timesteps to stack
        stacked_timesteps = [timestep - i * self.frame_stack_stride 
                            for i in range(self.num_stacked_frames - 1, -1, -1)]
        
        # Collect frames for stacking
        rgb_frames = []
        depth_frames = []
        
        for t in stacked_timesteps:
            rgb = torch.from_numpy(obs_group['rgb_image'][t]).to(self.device).float()
            depth = torch.from_numpy(obs_group['depth_image'][t]).to(self.device).float()
            ...
            depth = torch.nan_to_num(depth, nan=6.0)
            depth = torch.where(torch.isinf(depth), torch.tensor(6.0, device=self.device), depth)
            depth = torch.clamp(depth, min=0.0, max=6.0)
            
            rgb_frames.append(rgb)
            depth_frames.append(depth)
        
        # Stack along channel dimension: (num_stacked_frames * C, H, W)
        rgb_stacked = torch.cat(rgb_frames, dim=0)  # (num_stacked_frames * 3, H, W)
        depth_stacked = torch.cat(depth_frames, dim=0)  # (num_stacked_frames * 1, H, W)
        
        # Process proprioceptive data (only from current timestep)
        proprio_tensors = [torch.from_numpy(np.atleast_1d(obs_group[key][timestep])).to(self.device).float() 
                          for key in self.proprioceptive_keys]
        proprioceptive = torch.cat(proprio_tensors, dim=0)
