    ...
    def step(self, action):
        ...
        if self.episode_rewards is None:
            if reward_tensor.ndim > 0:
                ...
                self.episode_rewards = torch.zeros_like(
                    reward_tensor.squeeze())
            else:
                ...
                self.episode_rewards = torch.zeros(
                    num_envs, dtype=torch.float32, device='cpu')
        ...
        if done.any():
            ...
            try:
                ep_info = info.get("episode", None) if isinstance(
                    info, dict) else None
                if isinstance(ep_info, dict):
                    keys_lower = {k: k.lower() for k in ep_info.keys()}
                    candidates = [
                        k for k, kl in keys_lower.items() if "is_success" in kl]
                    if not candidates:
                        candidates = [
                            k for k, kl in keys_lower.items() if "success" in kl]
                    ...
                    for success_key in candidates:
                        ...
                        if sv.numel() == finished_mask_cpu.numel():
                            aligned_sv = sv.view_as(finished_mask_cpu)[
                                finished_mask_cpu]
                        ...
                        if aligned_sv is not None:
                            success_tensor = aligned_sv if aligned_sv.dtype == torch.bool else (
                                aligned_sv.float() > 0.5)
                            if success_tensor.ndim == 0:
                                successes_found.append(
                                    bool(success_tensor.item()))
                            else:
                                for success_val in success_tensor:
                                    successes_found.append(
                                        bool(success_val.item()))
                            break
            except Exception:
                pass
